...
namespace BriLib
{
    public class Quadtree<T>
    {
        ...
        public Quadtree(AxisAlignedBoundingBox boundingBox, int maxObjectsPerNode)
        {
        ...
        public Quadtree(float centerX, float centerY, float radius, int maxObjectsPerNode)
            : this(new AxisAlignedBoundingBox(centerX, centerY, radius), maxObjectsPerNode) { }
        ...
        {
            return GetRange(new AxisAlignedBoundingBox(x, y, radius));
        }

        public IEnumerable<T> GetRange(AxisAlignedBoundingBox bounds)
        {
        ...
        private void Subdivide()
        {
            ...
            var nwBox = new AxisAlignedBoundingBox(x - quarterRadius, y + quarterRadius, quarterRadius);
            _subtrees[(int)Quadrant.NorthWest] = new Quadtree<T>(nwBox, _maxObjectsPerNode);

            var neBox = new AxisAlignedBoundingBox(x + quarterRadius, y + quarterRadius, quarterRadius);
            _subtrees[(int)Quadrant.NorthEast] = new Quadtree<T>(neBox, _maxObjectsPerNode);

            var seBox = new AxisAlignedBoundingBox(x + quarterRadius, y - quarterRadius, quarterRadius);
            _subtrees[(int)Quadrant.SouthEast] = new Quadtree<T>(seBox, _maxObjectsPerNode);

            var swBox = new AxisAlignedBoundingBox(x - quarterRadius, y - quarterRadius, quarterRadius);

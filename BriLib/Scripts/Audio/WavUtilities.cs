...
namespace BriLib
{
  public class AudioCaptureManager : Singleton<AudioCaptureManager>
  {
    ...
    public async Task<byte[]> GetNextAudioFromMicrophone()
    {
      if (isRecording)
      {
      ...
      byte[] wavData = await Task.Run(() => EncodeSamplesToWAV(samples, channels, frequency));
      audioData.Clear();
      return wavData;
    }
    ...
    public byte[] EncodeSamplesToWAV(float[] samples, int channels, int sampleRate)
    {
